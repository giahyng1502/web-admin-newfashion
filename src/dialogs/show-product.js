...
const ProductDialog = ({ open, onClose, product, colors ,dispatch,showNotification}) => {
    ...
    const handleFileChangeForColor = async (event, index) => {
        ...
        if (file) {
            ...
            if (imageUrl.length > 0) {
                setColorImages(prev =>
                    prev.map((c, i) => (i === index ? { ...c, imageColor: imageUrl[0] } : c))
                );
            }
        }
    };
    ...
    const handleEditColorName = (index, name) => {
        setColorImages(prev => prev.map((c, i) => (i === index ? { ...c, nameColor: name } : c)));
    };

    // Thêm màu sắc mới
    const handleAddColor = () => {
        if (newColor.nameColor && newColor.imageColor) {
            setColorImages(prev => [...prev, { ...newColor, _id: new ObjectId().toString() }]);
        ...
        <Dialog open={open} onClose={onClose} fullWidth maxWidth="md">
            <div style={{ backgroundColor: colors.primary[400] }}>
                ...
                <DialogContent dividers>
                    ...
                    {/* Màu sắc */}
                    <Typography variant="subtitle1" mt={2}>Màu sắc:</Typography>
                    <Box display="flex" gap={2} flexWrap="wrap">
                        {colorImages.map((c, index) => (
                            <Box key={c._id} position="relative" textAlign="center">
                                ...
                                <IconButton
                                    size="small"
                                    sx={{ position: "absolute", top: -5, right: -5, backgroundColor: colors.primary[400] }}
                    ...
                    {/* Thêm màu mới */}
                    <Typography variant="subtitle1" mt={2}>Thêm màu sắc:</Typography>
                    <Box display="flex" gap={2} alignItems="center">
                        <TextField
                            ...
                            onChange={(e) => setNewColor(prev => ({ ...prev, nameColor: e.target.value }))}
                        />
                        <Button variant="contained" component="label">
                            Chọn ảnh
                            <input type="file" hidden accept="image/*" onChange={handleFileChangeForNewColor} />
                        </Button>
                        {newColor.imageColor && (
                            <img src={newColor.imageColor} alt="new-color" width={50} height={50} style={{ borderRadius: 5 }} />
                        )}
                    ...
                    {/* Ảnh sản phẩm */}
                    <Typography variant="subtitle1" mt={2}>Ảnh sản phẩm:</Typography>
                    <Box display="flex" gap={2} flexWrap="wrap">
                        {images.length > 0 ? (
                            images.map((img, index) => (
                                <Box key={index} position="relative">
                                    <img src={img} alt={`product-${index}`} width={100} height={100} style={{ borderRadius: 5 }} />
                                    <IconButton
                                        size="small"
                                        sx={{ position: "absolute", top: 0, right: 0, backgroundColor: colors.primary[400] }}
                    ...
                    <Box display="flex" mt={2} alignItems="center">
                        <Button variant="contained" component="label">
                            Chọn ảnh sản phẩm
                            <input type="file" hidden accept="image/*" onChange={handleFileChangeForProduct} />
                        </Button>
                    </Box>
                </DialogContent>
                <DialogActions>
                    ...
                    <Button onClick={updateProduct2} color="primary" variant="contained">
                        Cập nhập
                    </Button>
                </DialogActions>
            </div>
