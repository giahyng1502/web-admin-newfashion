...
import {Box, Button, Paper, Select, MenuItem, useTheme, TextField, IconButton, InputBase} from "@mui/material";
...
const handleRoleChange = async (userId, newRole, dispatch, showNotification) => {
    try {
        ...
        renderCell: (params) => {
            ...
            return (
                <Select
                    value={params.value}
                    onChange={(event) =>
                        handleRoleChange(params.row._id, event.target.value, dispatch, showNotification)
                    }
                    sx={{ width: "100%" }}
                    variant="filled"
...
export default function DataTable({setSearch, setSortModel,sortModel,isLoading, rows, pageSize, setPageSize, page, setPage, rowCount, handleOpenAddUser }) {
