...
import {getAllSubCate, getAllSubCateName, getSubcate} from "../../redux/reducer/subcateReducer";
...
function AddProduct() {
    ...
    const validateForm = () => {
        ...
        if (!product.stock.trim() || isNaN(product.stock) || Number(product.stock) <= 0) {
        ...
        if (!product.cost.trim() || isNaN(product.cost) || Number(product.cost) <= 0) {
            setDialogContent("Vui lòng nhập giá nhập hợp lệ!");
            setDialogOpen(true);
            return false;
        }
        if (!product.price.trim() || isNaN(product.price) || Number(product.price) <= 0) {
    ...
    return (
        <>
            <ValueComfirm open={dialogOpen} content={dialogContent} onClose={() => setDialogOpen(false)} />
            <DescriptionDialog open={isDescDialog} onSave={(data)=>{
                setProduct((prev) => ({
                    ...prev,
                    description: data,
                }));
        ...
        >
            ...
            <Grid container spacing={2}>
                ...
                {product.image.map((item, index) => (
                    <Grid item xs={2} key={index}>
                        <Card sx={{ position: "relative" }}>
                            <img src={item} alt={`image-${index}`} width="100%" height="100%" style={{ objectFit: "cover" }} />
                            <IconButton sx={{ position: "absolute", top: 5, right: 5 }} onClick={() => handleDeleteImage(index)}>
                ...
                <Grid item xs={12}>
                    ...
                    <Button variant="contained" component="label">
                        Thêm ảnh sản phẩm
                        <input type="file" hidden accept="image/*" multiple onChange={handleProductImagesChange} />
                    </Button>
                </Grid>
                {/* Thông tin sản phẩm */}
                ...
                <Grid item xs={6}>
                    <CustomAutocomplete subcate={subcate} onSelect={(selectedSubCate) => {
                        setProduct(prev => ({
                            ...prev,
                ...
                <Grid item xs={2}>
                    ...
                    }} variant="contained">Description</Button>
                </Grid>

                {/* Danh sách màu đã thêm */}
                {product.color.map((item, index) => (
                    <Grid item key={index} xs={2}>
                        <Card elevation={8} sx={{ position: "relative", padding: 2, textAlign: "center" }}>
                            <img src={item.imageColor} alt="image-color" width={50} height={50} style={{ borderRadius: 5 }} />
                ...
                {/* Thêm màu mới */}
                <Grid item xs={2}>
                    <Card elevation={8} sx={{ backgroundColor: colors.primary[400], borderRadius: 2, p: 2, textAlign: "center", position: "relative" }}>
                        {newColor.imageColor ? (
                            <Box sx={{ position: "relative" }}>
                                <img src={newColor.imageColor} alt="selected-color" width={80} height={80} style={{ borderRadius: 5 }} />
                                <IconButton
                                    sx={{ position: "absolute", top: 5, right: 5 }}
                                    onClick={() => setNewColor(prev => ({ ...prev, imageColor: "" }))}
                                >
                                    <DeleteIcon color="error" />
                                </IconButton>
                            </Box>
                            ...
                            <IconButton component="label" sx={{ width: "100%" }}>
                                <AddIcon sx={{ height: 80, width: 80 }} />
                                <input type="file" hidden accept="image/*" onChange={handleColorImageChange} />
                            </IconButton>
                        )}
                        ...
                        <TextField
                            ...
                            onChange={(e) => setNewColor({ ...newColor, nameColor: e.target.value })}
                            fullWidth
                            sx={{ mt: 1 }}
                        />

                        <Button variant="contained" color="secondary" sx={{ mt: 1, width: "100%" }} onClick={handleAddColor}>
                            OK
                        </Button>
                    </Card>
                </Grid>
            ...
            <Divider sx={{my:2}} />
                <Button onClick={uploadProduct} variant={'contained'} sx={{float:'right'}} color={'primary'}>Lưu</Button>
        </Paper>
        </>
    );
}
