...
export const utilOrder = (orders) => {
    return orders.map(order => ({
        ...
        productDesc: order.item.map(data => `- X${data.quantity} ${data.productName} màu ${data.color.nameColor} cỡ ${data.size} `).join("\n"),
...
function Order(props) {
    ...
    useEffect( () => {
        setLoading(true)
        getAllOrder(dispatch).then(response => {setLoading(false)})
        setLoading(false)
    }, [dispatch]);
    return (
        <Box p={2}>
